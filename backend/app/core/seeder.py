...
from ..models.skill import Skill, SkillCategory, SKILL_DEFINITIONS
from ..models.inventory import Item, ItemType, ItemRarity, EquipmentSlot, STARTER_ITEMS
from ..models.world import Zone, Location
from ..models.economy import NPCMerchant, CraftingRecipe
...
async def seed_npc_merchant(session: AsyncSession, general_store_location: Location):
    ...
    result = await session.execute(select(NPCMerchant).where(NPCMerchant.name == "Merchant Tom"))
...
async def seed_chat_channels(session: AsyncSession):
    ...
    if existing_channels:
        print(f"Chat channels already exist ({len(existing_channels)} found), skipping...")
        return
