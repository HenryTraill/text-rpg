...
class HealthChecker:
    ...
    async def get_health_status(self, include_details: bool = False) -> Dict[str, Any]:
        ...
        # Process results
        health_data["checks"]["database"] = checks[0] if not isinstance(checks[0], Exception) else {"status": "error", "error": str(checks[0])}
        health_data["checks"]["redis"] = checks[1] if not isinstance(checks[1], Exception) else {"status": "error", "error": str(checks[1])}
        health_data["checks"]["system"] = checks[2] if not isinstance(checks[2], Exception) else {"status": "error", "error": str(checks[2])}
