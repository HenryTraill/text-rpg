...
class TestHealthChecker:
    ...
    async def test_get_health_status_healthy(self, mock_health_checker):
        ...
        mock_health_checker._check_database = AsyncMock(return_value={
            "status": "healthy",
            "connection_time": 0.1
        })
        mock_health_checker._check_redis = AsyncMock(return_value={
            "status": "healthy",
            "connection_time": 0.05
        })
        mock_health_checker._check_system_resources = AsyncMock(return_value={
            "status": "healthy",
            "cpu_percent": 25.0
        })
    ...
    async def test_get_health_status_with_details(self, mock_health_checker):
        ...
        mock_health_checker._get_detailed_metrics = AsyncMock(return_value={
            "application": {"name": "Test RPG API"}
        })
    ...
    async def test_is_ready_healthy(self, mock_health_checker):
        ...
        mock_health_checker.get_health_status = AsyncMock(return_value={
            "status": "healthy"
        })
