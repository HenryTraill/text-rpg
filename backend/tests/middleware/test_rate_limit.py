...
class TestRateLimitMiddleware:
    ...
    async def test_get_client_info_authenticated_user(self, rate_limiter, mock_request):
        ...
        with patch.object(rate_limiter, '_extract_user_id_from_token', return_value="user123"):
    ...
    async def test_get_client_info_anonymous_user(self, rate_limiter, mock_request):
        ...
        with patch.object(rate_limiter, '_extract_user_id_from_token', return_value=None):
            client_id, rate_limit = await rate_limiter._get_client_info(mock_request)
    ...
    async def test_check_rate_limit_allowed(self, rate_limiter):
        ...
        allowed, remaining, reset_time = await rate_limiter._check_rate_limit(client_id, rate_limit)
    ...
    async def test_check_rate_limit_exceeded(self, rate_limiter):
        ...
        mock_pipeline = Mock()
        mock_pipeline.execute.return_value = [None, 100, None, None]  # 100 current requests
        rate_limiter.redis_client.pipeline.return_value = mock_pipeline
    ...
    async def test_dispatch_request_allowed(self, rate_limiter, mock_request):
        ...
        with patch.object(rate_limiter, '_get_client_info', return_value=("user:test", 100)), \
             patch.object(rate_limiter, '_check_rate_limit', return_value=(True, 99, int(time.time()) + 60)):
    ...
    async def test_dispatch_rate_limit_exceeded(self, rate_limiter, mock_request):
        ...
        with patch.object(rate_limiter, '_get_client_info', return_value=("user:test", 100)), \
             patch.object(rate_limiter, '_check_rate_limit', return_value=(False, 0, reset_time)):
...
class TestRateLimitMiddlewareIntegration:
    ...
    async def test_authenticated_vs_anonymous_limits(self):
        ...
        with patch('redis.from_url') as mock_redis_factory:
            ...
            with patch.object(middleware, '_extract_user_id_from_token', return_value="user123"):
                client_id, rate_limit = await middleware._get_client_info(mock_request_auth)
            ...
            with patch.object(middleware, '_extract_user_id_from_token', return_value=None):
                client_id, rate_limit = await middleware._get_client_info(mock_request_anon)
    ...
    async def test_rate_limit_headers_format(self):
        ...
        with patch('redis.from_url') as mock_redis_factory:
            ...
            with patch.object(middleware, '_get_client_info', return_value=("ip:127.0.0.1", 20)), \
                 patch.object(middleware, '_check_rate_limit', return_value=(True, 19, reset_time)):
