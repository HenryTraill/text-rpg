...
class TestSecurityMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_validate_request_security_clean(self, security_middleware, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_validate_request_security_suspicious_url(self, security_middleware, mock_request):
        """Test request security validation with suspicious URL patterns."""
        mock_request.url.__str__ = Mock(return_value="http://localhost:8000/api/v1/test?q=<script>alert('xss')</script>")
    ...
    @pytest.mark.asyncio
    async def test_validate_request_security_suspicious_header(self, security_middleware, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_validate_request_security_oversized_header(self, security_middleware, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_validate_request_security_bot_detection(self, security_middleware, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_validate_request_security_missing_content_type(self, security_middleware, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_handle_preflight_allowed_origin(self, security_middleware, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_handle_preflight_disallowed_origin(self, security_middleware, mock_request):
    ...
    @pytest.mark.asyncio
    async def test_dispatch_security_violations(self, security_middleware, mock_request):
        """Test request handling with security violations."""
        # Mock security validation to return violations
        with patch.object(security_middleware, '_validate_request_security', return_value=["test_violation"]):
            async def call_next(request):
                return Response("OK", status_code=200)
...
class TestSecurityMiddlewareIntegration:
    ...
    async def test_malicious_request_blocking(self):
        ...
        request.url.path = "/api/v1/test"
        request.method = "GET"
        request.url.__str__ = Mock(return_value="http://localhost:8000/api/v1/test?q=<script>alert('xss')</script>")
        request.client.host = "127.0.0.1"
        request.headers = {}
